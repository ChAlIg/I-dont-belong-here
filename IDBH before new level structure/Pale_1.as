package  {
	...
	public class Pale_1 extends MovieClip {
		...
		public function Pale_1(main:Game, where:Level, X: int, Y: int, theMission: String = "idle"): void {
			this.x = X;
			this.y = Y;
			level = where;
			game = main;
			...
			for (i = 0; i < game.playerList.length; ++i) {
				...
				if (distance < 300) {
					...
					for (j = speed; j < distance; j += speed) {
						testPoint.x += stepX;
						testPoint.y += stepY;
						testPointGlobal = level.localToGlobal(testPoint);
						if (level.walls.hitTestPoint(testPointGlobal.x, testPointGlobal.y, true)) {
							token = false;
							break;
						}
					}
